...
export default function handler(
  ...
  const { link, id = 1, projectId = 1, statusId, currentSubdomain, newSubdomain, subdomainId, framework, port } = req.query;
  ...
  pb.collection('projects').update(projectId, { subdomain: newSubdomain })
    .then(output => { console.log(output) })
    .catch(e => { console.log(e) })

  // Update the subdomain in the 'subdomains'
  pb.collection("subdomains").update(subdomainId, { name: newSubdomain })
    .then(output => { console.log(output) })
    .catch(e => { console.log(e) })
  ...
  executeCommandChild(
    `sh`, [`${scriptLocation}`, `${newSubdomain}`, `${port}`, `${id}`]
  ).then((output: any) => {
    log(chalk.blue("Nginx entry created ", output.stdout, output.stderr));

    // Remove nginx entry
    executeCommandChild('rm', ['-f', `/etc/nginx/techsapien.d/${id}/${currentSubdomain}.techsapien.dev.conf`])
